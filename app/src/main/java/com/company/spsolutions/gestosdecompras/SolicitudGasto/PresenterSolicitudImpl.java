package com.company.spsolutions.gestosdecompras.SolicitudGasto;
/**
 * Created by coralRodriguez on 28/03/19.
 */

public class PresenterSolicitudImpl {
    /*
    *PresenterSolicitudes manejara la vista y los cambios que debe sufrir
    * search es el método que busca en el modelo los registros de las solicitudes
    * edit gestiona si se esta editando o añadiendo una solicitud y notifica a la vista y al modelo
     */

    public PresenterSolicitudImpl(SolicitudActivity view, PresenterSolicitud delegate) {

    }

    public void search(){}
    public void edit(){}
}
